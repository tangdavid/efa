...
function( A, y , verbose=TRUE ){
	...
	while ( it < 100 & ( abs(l_dif) >= 10^-4 | (abs(l_dif)<10^-2 & l_dif < 0)) ){
		...
		# ANDY:
		AI <- 0.5*( AI/2 + t(AI)/2 )
		minlam	<- min( eigen( AI, symm=T )$val )
		if( minlam < 1e-12 ){
            if(verbose) print( paste( 'Minlam=', minlam ) )
			AI	<- AI + diag(nrow(AI))*( abs(minlam)+1e-12 )
		}
		# end
		...
		if ( l_dif > 1 ) Var <- Var + 0.316*(solve(AI) %*% s)
			...
			print( quantile( eigen( AI, symm=T )$val ) )
			print( quantile( AI - t(AI) ) )
			print( 'meltdown 2' )
		})
		#### ANDY:
	...
	S <- 0.5*S
	#Sinv <- solve(S)
	Sinv <- solve(S+1e-10*diag(nrow(S)))
	...
	if( verbose ) cat( "Vp\t",sum(Var),'\t',SE.p,'\n',sep="")
	...
	for( i in 1:r ) {
		...
		if(verbose) cat( "V(G",i,")/Vp\t",Var[i]/sum(Var),'\t',SE.i[i],'\n',sep='')
	}
	
	return( list( Var=Var, SE=SE.i ))
}
